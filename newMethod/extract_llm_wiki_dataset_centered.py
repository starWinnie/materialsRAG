...
STAGE_TAXONOMY = {
    ...
    "stage_types": [
        ...
        {"stage_type": "ablation_study", "description": "Ablating model components or design choices."},
...
model_evaluation, ablation_study, candidate_generation, candidate_screening,
...
- Extract one DatasetUse event for each dataset actually used in each stage.
  These event records will later be merged into one DatasetUse page per Dataset.
- Create separate DatasetUse records for multiple datasets in one stage.
- Create exactly one DatasetUse event for a given dataset in a given stage. If several
...
GENERIC_LOCAL_DATASET_SLUGS = {
    ...
    "screening_dataset",
    "synthetic_dataset",
    "paper_specific_dataset",
}


def dataset_name_candidates(dataset: dict[str, Any]) -> list[str]:
    """Return all names that can identify the same Dataset node.

    The new Wiki is dataset-centered, so Dataset deduplication must happen before
    DatasetUse aggregation. We therefore consider canonical_name and raw_names
    together, then apply the hand-written alias table for common public datasets.
    """
    return unique_strings([dataset.get("canonical_name")], dataset.get("raw_names", []))


def canonical_dataset_slug(dataset: dict[str, Any]) -> str:
    """Normalize dataset names into a stable deduplication slug.

    This is intentionally stricter than normal display-name formatting:
    - common public database aliases collapse to one slug, e.g. MP variants;
    - exact canonical/raw-name duplicates across papers collapse;
    - generic local names such as "training dataset" remain paper-local to avoid
      falsely merging unrelated author-created datasets.
    """
    slugs = [slugify(name) for name in dataset_name_candidates(dataset)]
    slugs = [slug for slug in slugs if slug]
    if not slugs:
        slugs = [slugify(dataset.get("canonical_name") or "unnamed")]
    for slug in slugs:
        if slug in DATASET_ALIAS_GROUPS:
            return DATASET_ALIAS_GROUPS[slug][0]
    canonical_slug = slugs[0]
    return DATASET_ALIAS_GROUPS.get(canonical_slug, (canonical_slug, dataset.get("canonical_name")))[0]
...
def dataset_local_identity(dataset: dict[str, Any], paper_id: str) -> str:
    canonical_slug = canonical_dataset_slug(dataset)
    if dataset["dataset_type"] in {"derived_subset", "paper_specific"} and canonical_slug in GENERIC_LOCAL_DATASET_SLUGS:
        return f"local:{paper_id}:{dataset['dataset_type']}:{canonical_slug}"
...
def consolidate_results(normalized_results: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], ...]:
    """Merge paper-level extractions into a dataset-centered Wiki graph.

    The original extractor stored one DatasetUse per paper/stage/dataset event.
    This version keeps those events as usage_records, but exposes exactly one
    DatasetUse node/page for each deduplicated Dataset. This matches the
    recommendation scenario better: retrieval ranks datasets and their usage
    profiles, not dozens of near-duplicate usage-event pages.
    """
    papers, tasks, stages = [], [], []
    raw_event_uses: list[dict[str, Any]] = []
    dataset_by_identity: dict[str, dict[str, Any]] = {}
    canonical_id_by_old_id: dict[str, str] = {}

    # 1) Strictly deduplicate Dataset nodes before building DatasetUse pages.
    for result in normalized_results:
        ...
        for dataset in result["datasets"]:
            identity = dataset_local_identity(dataset, paper_id)
            clean_dataset = {key: value for key, value in dataset.items() if not key.startswith("_")}
            old_dataset_id = clean_dataset["dataset_id"]
            clean_dataset["dataset_id"] = dataset_id_from_identity(identity, clean_dataset, paper_id)
        ...
        raw_event_uses.extend(result["dataset_uses"])

    # 2) Rewrite source_dataset_id after all Dataset IDs have been canonicalized.
    ...
    # 3) Convert event-level DatasetUse records into usage_records.
    #    Duplicate events from the same paper/stage/dataset are merged first.
    merged_records: dict[tuple[str, str, str], dict[str, Any]] = {}
    record_order: list[tuple[str, str, str]] = []
    for use in raw_event_uses:
        use = dict(use)
        use["dataset_id"] = canonical_id_by_old_id.get(use["dataset_id"], use["dataset_id"])
        key = (use["paper_id"], use["stage_id"], use["dataset_id"])
        if key not in merged_records:
            use["usage_record_id"] = f"UR_{use['paper_id']}_{use['stage_id'].split('_')[-1]}_{use['dataset_id'].removeprefix('D_')}"
            merged_records[key] = use
            record_order.append(key)
            continue
        target = merged_records[key]
        target["used_fields"] = unique_strings(target.get("used_fields", []), use.get("used_fields", []))
        target["filter_conditions"] = unique_strings(target.get("filter_conditions", []), use.get("filter_conditions", []))
        target["evidence"] = list(target.get("evidence", [])) + list(use.get("evidence", []))
    ...
    usage_records = [merged_records[key] for key in record_order]
    for record in usage_records:
        if record.get("sample_count") is None:
            continue
        evidence_text = " ".join(str(item.get("text") or "") for item in record.get("evidence", []))
        evidence_digits = re.sub(r"(?<=\d)[,\s](?=\d)", "", evidence_text)
        if str(record["sample_count"]) not in evidence_digits:
            record["sample_count"] = None

    # 4) Build one DatasetUse node per deduplicated Dataset.
    records_by_dataset: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for record in usage_records:
        records_by_dataset[record["dataset_id"]].append(record)
    dataset_uses: list[dict[str, Any]] = []
    for dataset_id, records in sorted(records_by_dataset.items()):
        confidences = [float(record.get("confidence", 0.5)) for record in records]
        dataset_uses.append({
            "dataset_use_id": f"DU_{dataset_id.removeprefix('D_')}",
            "dataset_id": dataset_id,
            "paper_ids": sorted({record["paper_id"] for record in records}),
            "task_ids": sorted({record["task_id"] for record in records}),
            "stage_ids": sorted({record["stage_id"] for record in records}),
            "usage_roles": unique_strings([record.get("usage_role") for record in records]),
            "purposes": unique_strings([record.get("purpose") for record in records]),
            "used_fields": unique_strings(*(record.get("used_fields", []) for record in records)),
            "filter_conditions": unique_strings(*(record.get("filter_conditions", []) for record in records)),
            "construction_methods": unique_strings([record.get("construction_method") for record in records]),
            "sample_counts": sorted({
                record["sample_count"] for record in records
                if isinstance(record.get("sample_count"), int)
            }),
            "evidence": [
                item for record in records for item in record.get("evidence", [])
            ],
            "usage_records": records,
            "confidence": round(min(confidences) if confidences else 0.5, 4),
        })
...
def build_edges(papers: list[dict[str, Any]], tasks: list[dict[str, Any]],
    ...
    for use in dataset_uses:
        ...
        # DatasetUse is dataset-centered. The node connects to its Dataset once,
        # while every paper/task/stage usage event is represented by a separate
        # edge carrying the same DatasetUse target.
        edge_paper_id = use["paper_ids"][0] if use.get("paper_ids") else "P000"
        add(edge_paper_id, "DatasetUse", use["dataset_use_id"], "USES_DATASET", "Dataset", use["dataset_id"], confidence)
        for record in use.get("usage_records", []):
            record_confidence = record.get("confidence", confidence)
            add(record["paper_id"], "Paper", record["paper_id"], "EVIDENCES_DATASET_USE",
                "DatasetUse", use["dataset_use_id"], record_confidence)
            add(record["paper_id"], "Task", record["task_id"], "HAS_DATASET_USE",
                "DatasetUse", use["dataset_use_id"], record_confidence)
            add(record["paper_id"], "Stage", record["stage_id"], "HAS_DATASET_USE",
                "DatasetUse", use["dataset_use_id"], record_confidence)
    for dataset in datasets:
...
def build_pages(root: Path, papers: list[dict[str, Any]], tasks: list[dict[str, Any]],
    ...
    for use in uses:
        ...
        for record in use.get("usage_records", []):
            record_view = {**record, "dataset_use_id": use["dataset_use_id"], "dataset_id": use["dataset_id"]}
            uses_by_task[record["task_id"]].append(record_view)
            uses_by_stage[record["stage_id"]].append(record_view)
    expected_pages = {
    ...
    for use in uses:
        dataset = dataset_by_id[use["dataset_id"]]
        records = sorted(
            use.get("usage_records", []),
            key=lambda item: (item.get("paper_id", ""), item.get("stage_id", ""), item.get("usage_role", "")),
        )
        record_lines = []
        for record in records:
            paper = paper_by_id.get(record["paper_id"], {"title": record["paper_id"]})
            task = task_by_id.get(record["task_id"])
            stage = stage_by_id.get(record["stage_id"])
            task_label = f"{task['task_name_raw']} (`{task['task_id']}`)" if task else record["task_id"]
            stage_label = (
                f"{stage['stage_name_raw']} (`{stage['stage_type']}`, `{stage['stage_id']}`)"
                if stage else record["stage_id"]
            )
            record_lines.append(
                "\n".join([
                    f"### {record.get('usage_record_id', record['paper_id'])}",
                    "",
                    f"- Paper: `{record['paper_id']}` — {paper.get('title')}",
                    f"- Task: {task_label}",
                    f"- Stage: {stage_label}",
                    f"- Usage role: {record.get('usage_role')}",
                    f"- Purpose: {record.get('purpose')}",
                    f"- Used fields: {', '.join(record.get('used_fields') or []) or 'Not stated'}",
                    f"- Filter conditions: {', '.join(record.get('filter_conditions') or []) or 'Not stated'}",
                    f"- Construction method: {record.get('construction_method') or 'Not stated'}",
                    f"- Sample count: {record.get('sample_count') if record.get('sample_count') is not None else 'Not stated'}",
                    f"- Confidence: {record.get('confidence')}",
                    "",
                    "Evidence:",
                    md_evidence(record.get("evidence", []), record["paper_id"]),
                ])
            )
        body = [
            f"# Dataset Use: {dataset['canonical_name']}", "",
            f"- DatasetUse ID: `{use['dataset_use_id']}`",
            f"- Dataset: {dataset['canonical_name']} (`{dataset['dataset_id']}`)",
            f"- Papers: {', '.join(use.get('paper_ids', [])) or 'None'}",
            f"- Usage records: {len(records)}",
            "", "## Usage roles", "", md_list(use.get("usage_roles", [])),
            "", "## Purposes", "", md_list(use.get("purposes", [])),
            "", "## Used fields", "", md_list(use.get("used_fields", [])),
            "", "## Construction methods", "", md_list(use.get("construction_methods", [])),
            "", "## Filter conditions", "", md_list(use.get("filter_conditions", [])),
            "", "## Sample counts", "", md_list(use.get("sample_counts", [])),
            "", "## Availability", "", f"- Dataset: {dataset['availability']}", f"- Recommendable: {str(dataset['recommendable']).lower()}",
            "", "## Confidence", "", str(use["confidence"]),
            "", "## Usage records", "", "\n\n".join(record_lines) if record_lines else "None", "",
            "## Aggregated evidence", "", md_evidence(use.get("evidence", []), ""), "",
        ]
    ...
    for dataset in datasets:
        ...
        observed_tasks = unique_strings([
            task_by_id[record["task_id"]]["task_name_raw"]
            for record in dataset_records
            if record.get("task_id") in task_by_id
        ])
        observed_stages = unique_strings([
            stage_by_id[record["stage_id"]]["stage_type"]
            for record in dataset_records
            if record.get("stage_id") in stage_by_id
        ])
        usage_lines = []
        for use in dataset_uses:
            for record in use.get("usage_records", []):
                paper, stage = paper_by_id[record["paper_id"]], stage_by_id[record["stage_id"]]
                usage_lines.append(f"{paper['paper_id']} ({paper['title']}): {record['usage_role']} in {stage['stage_name_raw']} — {record['purpose']}")
...
def build_registries(root: Path, tasks: list[dict[str, Any]], datasets: list[dict[str, Any]],
    ...
    for use in uses:
        ...
        use_count[use["dataset_id"]] += len(records)
        for record in records:
            papers_by_dataset[use["dataset_id"]].add(record["paper_id"])
    dataset_registry = [{
        **dataset,
...
def validate_graph(papers: list[dict[str, Any]], tasks: list[dict[str, Any]],
    ...
    for use in uses:
        if use["dataset_id"] not in dataset_ids:
            errors.append({"type": "missing_dataset", "record_id": use["dataset_use_id"]})
        if not use.get("evidence"):
            errors.append({"type": "missing_evidence", "record_id": use["dataset_use_id"]})
        if not use.get("usage_records"):
            errors.append({"type": "missing_usage_records", "record_id": use["dataset_use_id"]})
        for record in use.get("usage_records", []):
            for field, valid, kind in (("task_id", task_ids, "task"), ("stage_id", stage_ids, "stage"), ("dataset_id", dataset_ids, "dataset")):
                if record[field] not in valid:
                    errors.append({"type": f"missing_{kind}", "record_id": record.get("usage_record_id", use["dataset_use_id"])})
    for dataset in datasets:
...
def main() -> int:
    ...
    output_root = args.output_root or script_path.parent / "LLMWiki_dataset_centered"
    ...
    manifest = {
        "schema_version": "dataset-centered-1.0", "created_at": utc_now(),
