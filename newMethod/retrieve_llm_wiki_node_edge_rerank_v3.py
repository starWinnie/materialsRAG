...
def candidate_intent_coverage(
    ...
    """Map a Dataset candidate to coarse needs using only current evidence."""
    ...
    text = _text_blob(
        ...
        [task.get("objective") for task in tasks],
        [task.get("material_scope", []) for task in tasks],
        [stage.get("stage_type") for stage in stages],
    )
    ...
    property_values = [
        ...
        *dataset.get("available_fields", []),
        *[field for use in uses for field in use.get("used_fields", [])],
        *[task_prop for task in tasks for task_prop in task.get("target_properties", [])],
    ]
