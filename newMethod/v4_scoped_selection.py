...
def select_scoped(*, store, plan, question, ranked, max_results, candidate_top_k, beam_width=128):
    ...
    diagnostics = {"mode": "scoped", "algorithm": "role_scoped_recommendation_v4_4", "beam_width": beam_width,
                   ...
                   "selection_reason": "all_parsed_needs_supported" if all(v >= 1 for v in coverage) else "conditional_or_partial_recommendation"}
...
def scoped_answer(store, selected, diagnostics):
    ...
    for candidate in selected:
        ...
        suffix = f"；可替代 {alternative['alternative_to']}，不表示必须同时使用" if alternative else "；主组合"
