...
# -*- coding: utf-8 -*-
"""
Node-only Retrieval -> Intent-lane RRF -> Controlled DatasetUse Completion -> Edge RRF/Local Match -> One-stage Rerank -> Dataset Aggregation.
...
# Edge diversity controls. These are intentionally conservative starting values
# for evaluation; tune them on a validation set rather than per test case.
DEFAULT_MAX_EDGE_USES_PER_TASK = 5
DEFAULT_MAX_EDGE_USES_PER_DATASET = 3
DEFAULT_MAX_EDGE_USES_PER_TASK_DATASET = 2

# Edge-local semantic match is a lightweight auxiliary score computed from the
# DatasetUse-local facts (dataset name/properties/role/purpose/fields), excluding
# the full Task text. Set to 0 to disable.
DEFAULT_EDGE_LOCAL_WEIGHT = 0.25
...
DEFAULT_RERANK_INSTRUCT = (
    ...
    "selecting datasets needed to complete the materials R&D query. Judge the "
    "DatasetUse-local facts first: dataset identity, available properties, used "
    "fields, usage role, purpose, filters, and explicit constraints. Treat the "
    "Task/Stage context only as supporting context; a highly similar Task must "
    "not make a DatasetUse relevant when its own fields/purpose conflict with "
    "the requested property or data need. Treat each record as one possible role "
    "in a multi-dataset workflow, not as a dataset that must satisfy every need "
    "alone. For cross-property transfer learning, a pretraining source may have "
    "labels different from the target properties; do not reject it only for that "
    "reason. Prefer explicit evidence and penalize explicit conflicts."
)
...
class EdgeDiscovery:
    ...
    edge_local_score: float = 0.0
    edge_seed_score: float = 0.0
...
def build_stratified_queries(
    ...
    # inserted by rewrite_hyperedge_query; canonical parents remain in the plan
    # for downstream matching/coverage.
    material = plan.material_scope[0] if plan.material_scope else "materials"
    ...
    if properties:
        add(
            dense,
            dense_lanes,
            f"{material} {properties} materials dataset",
            "material_property",
        )
    ...
    if mechanisms:
        mechanism_query = f"{material} {mechanisms} materials dataset"
    ...
    if "elemental_fractions" in plan.input_representations:
        input_query = (
            f"{material} elemental fractions composition chemical formula "
        ...
        input_query = f"{material} {' '.join(plan.input_representations)} dataset"
        add(dense, dense_lanes, input_query, "input")
        add(bm25, bm25_lanes, input_query, "input")
        subtasks.append(input_query)
    ...
    for need in plan.dataset_role_needs:
        if need["role"] == "pretraining_source":
            ...
            query = (
                f"{material} {need['scale']} source dataset pretraining "
                f"{relation} materials properties"
            )
            ...
            query = f"{material} {need['scale']} {source_role} dataset {properties}"
...
def rewrite_hyperedge_query(
    ...
    core.material_scope = base.unique_strings([*specific_materials, *core.material_scope])
    core.target_properties = base.unique_strings([*specific_properties, *core.target_properties])
...
def expand_nodes_to_dataset_uses(
    ...
    """Controlled one-hop completion from nodes to DatasetUse edges.

    Task and Dataset seeds may create candidate edges. Stage seeds are auxiliary:
    they may only boost DatasetUse edges already discovered by Task/Dataset.
    This prevents generic workflow stages from generating a large noisy pool.
    """
    ...
    def make_node_hit(seed: dict[str, Any], document: base.SearchDocument) -> tuple[list[dict[str, Any]], dict[str, Any]]:
        uses = base.document_dataset_uses(store, document)
        degree = degree_cache.setdefault(document.doc_id, len(uses))
        specificity = node_specificity(total_edges, degree, floor=specificity_floor)
    ...
    primary_seeds = [seed for seed in node_seeds if seed.get("node_type") in {"Task", "Dataset"}]
    stage_seeds = [seed for seed in node_seeds if seed.get("node_type") == "Stage"]

    # Primary endpoints create candidates.
    for seed in primary_seeds:
        ...
        for use in uses:
            ...
            item = discovered.setdefault(use_id, EdgeDiscovery(dataset_use_id=use_id))
            item.node_hits.append(node_hit)
            item.edge_rrf_score += float(node_hit["edge_rrf_contribution"])
            expansion_trace.append({
                "dataset_use_id": use_id,
                "task_id": use["task_id"],
                "stage_id": use["stage_id"],
                "dataset_id": use["dataset_id"],
                "discovery_mode": "primary_create",
                **node_hit,
            })

    # Stage endpoints only reinforce already-discovered candidates.
    for seed in stage_seeds:
        ...
        for use in uses:
            ...
            expansion_trace.append({
                ...
                "stage_id": use["stage_id"],
                "dataset_id": use["dataset_id"],
                "discovery_mode": "stage_boost_only",
                **node_hit,
            })

    maximum = max((item.edge_rrf_score for item in discovered.values()), default=1.0)
    for item in discovered.values():
        item.normalized_edge_rrf_score = item.edge_rrf_score / maximum if maximum > 0 else 0.0
        item.edge_seed_score = item.normalized_edge_rrf_score

    return discovered, expansion_trace


def serialize_edge_local(store: base.WikiStore, use: dict[str, Any]) -> str:
    """Serialize only local DatasetUse/Dataset facts for auxiliary semantic match."""

    dataset = store.dataset_by_id[use["dataset_id"]]
    return "\n".join([
        f"Dataset: {dataset.get('canonical_name') or 'unknown'}",
        f"Dataset aliases: {_values(dataset.get('raw_names', []))}",
        f"Dataset material scope: {_values(dataset.get('material_scope', []))}",
        f"Dataset properties: {_values(dataset.get('available_properties', []))}",
        f"Dataset fields: {_values(dataset.get('available_fields', []))}",
        f"Usage role: {use.get('usage_role') or 'unknown'}",
...
def score_edge_local_semantics(
    client: OpenAI,
    embedding_model: str,
    store: base.WikiStore,
    question: str,
    ...
    """Add a local semantic score without reusing the full Task text.

    This is intentionally auxiliary. Endpoint-RRF remains the dominant signal.
    Set local_weight=0 to disable the extra embedding call.
    """
    ...
    if weight <= 0.0:
        for item in discovered.values():
            item.edge_local_score = 0.0
            item.edge_seed_score = item.normalized_edge_rrf_score
        return {"enabled": False, "weight": 0.0, "count": len(discovered)}

    edge_ids = sorted(discovered)
    texts = [question]
    texts.extend(
        serialize_edge_local(store, store.use_by_id[edge_id])
    ...
    for edge_id, similarity in zip(edge_ids, similarities):
        item = discovered[edge_id]
        # Clamp negative cosine to zero; most embedding similarities here are
        # positive and already in a useful 0..1 range.
        local = min(1.0, max(0.0, float(similarity)))
        item.edge_local_score = local
        item.edge_seed_score = (
            (1.0 - weight) * item.normalized_edge_rrf_score
            + weight * local
        )
    ...
    return {
        ...
        "weight": weight,
        "count": len(edge_ids),
        "min_local_score": float(min(similarities)) if len(similarities) else 0.0,
        "max_local_score": float(max(similarities)) if len(similarities) else 0.0,
    }
...
def rank_hyperedges_by_rrf(
    ...
    max_per_task: int,
    max_per_dataset: int,
    max_per_task_dataset: int,
) -> tuple[list[EdgeDiscovery], list[EdgeDiscovery], dict[str, Any]]:
    """Rank edges then enforce diversity before the irreversible rerank cutoff."""
    ...
    selected: list[EdgeDiscovery] = []
    dropped: list[EdgeDiscovery] = []
    drop_reasons: dict[str, str] = {}

    task_limit = max(0, max_per_task)
    dataset_limit = max(0, max_per_dataset)
    pair_limit = max(0, max_per_task_dataset)
    limit = max(1, edge_top_k)

    for item in ranked:
        use = store.use_by_id[item.dataset_use_id]
        task_id = str(use["task_id"])
        dataset_id = str(use["dataset_id"])
        pair = (task_id, dataset_id)

        reason = ""
        if task_limit and task_counts[task_id] >= task_limit:
            reason = "task_cap"
        elif dataset_limit and dataset_counts[dataset_id] >= dataset_limit:
            reason = "dataset_cap"
        elif pair_limit and pair_counts[pair] >= pair_limit:
            reason = "task_dataset_pair_cap"

        if reason:
            dropped.append(item)
            drop_reasons[item.dataset_use_id] = reason
            continue
        ...
        if len(selected) < limit:
            ...
            dropped.append(item)
            drop_reasons[item.dataset_use_id] = "outside_edge_top_k"

    diagnostics = {
        "max_per_task": task_limit,
        "max_per_dataset": dataset_limit,
        "max_per_task_dataset": pair_limit,
        "selected_count": len(selected),
        "drop_reasons": drop_reasons,
        "selected_task_counts": dict(task_counts),
        "selected_dataset_counts": dict(dataset_counts),
        "selected_task_dataset_counts": {
            f"{task_id}|{dataset_id}": count
            for (task_id, dataset_id), count in pair_counts.items()
        },
    }
...
def edge_trace(item: EdgeDiscovery, store: base.WikiStore) -> dict[str, Any]:
    ...
    return {
        ...
        "edge_local_score": item.edge_local_score,
        "edge_seed_score": item.edge_seed_score,
...
def run_node_edge_query(
    *,
    ...
    specificity_floor: float,
    edge_top_k: int,
    max_edge_uses_per_task: int,
    max_edge_uses_per_dataset: int,
    max_edge_uses_per_task_dataset: int,
    edge_local_weight: float,
    supporting_uses_per_dataset: int,
    retrieval_top_k: int,
    candidate_top_k: int,
    max_results: int,
    ...
    edge_local_diagnostics = score_edge_local_semantics(
        client=client,
        embedding_model=embedding_model,
        store=store,
        question=question,
        discovered=discovered,
        local_weight=edge_local_weight,
    )
    edge_rrf_top, edge_rrf_dropped, edge_diversity_diagnostics = rank_hyperedges_by_rrf(
        store=store,
        discovered=discovered,
        edge_top_k=edge_top_k,
        max_per_task=max_edge_uses_per_task,
        max_per_dataset=max_edge_uses_per_dataset,
        max_per_task_dataset=max_edge_uses_per_task_dataset,
    )
    ...
    if use_rerank:
        ...
        ranked_uses = sorted(
            edge_rrf_top,
            key=lambda item: item.edge_seed_score,
            reverse=True,
        )
        for position, item in enumerate(ranked_uses, start=1):
            item.rerank_score = item.edge_seed_score
            item.rerank_position = position
    ...
    trace = {
        "created_at": base.utc_now(),
        "pipeline": "node_only_lane_rrf_controlled_hyperedge_v2",
        "question": question,
        ...
        "retrieval_design": {
            ...
            "stage_can_create_edges": False,
            "edge_rrf_from_endpoint_node_ranks": True,
            "edge_local_semantic_auxiliary": edge_local_weight > 0,
        ...
        "edge_top_k": edge_top_k,
        "edge_local_diagnostics": edge_local_diagnostics,
        "edge_diversity_diagnostics": edge_diversity_diagnostics,
        "edge_rrf_top": [edge_trace(item, store) for item in edge_rrf_top],
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--max-edge-uses-per-task",
        type=int,
        default=DEFAULT_MAX_EDGE_USES_PER_TASK,
        help="进入 Edge Top-K 前每个 Task 最多保留的 DatasetUse 数；0 表示不限制",
    )
    parser.add_argument(
        "--max-edge-uses-per-dataset",
        type=int,
        default=DEFAULT_MAX_EDGE_USES_PER_DATASET,
        help="进入 Edge Top-K 前每个 Dataset 最多保留的 DatasetUse 数；0 表示不限制",
    )
    parser.add_argument(
        "--max-edge-uses-per-task-dataset",
        type=int,
        default=DEFAULT_MAX_EDGE_USES_PER_TASK_DATASET,
        help="同一 Task-Dataset 对最多保留的 DatasetUse 数；0 表示不限制",
    )
    parser.add_argument(
        "--edge-local-weight",
        type=float,
        default=DEFAULT_EDGE_LOCAL_WEIGHT,
        help="Edge-local embedding 相似度在粗排中的权重，0 可关闭额外 embedding",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    answer, trace = run_node_edge_query(
        ...
        max_edge_uses_per_task=args.max_edge_uses_per_task,
        max_edge_uses_per_dataset=args.max_edge_uses_per_dataset,
        max_edge_uses_per_task_dataset=args.max_edge_uses_per_task_dataset,
        edge_local_weight=args.edge_local_weight,
